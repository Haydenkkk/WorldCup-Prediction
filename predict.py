...
def get_result(world_cup, model, wmk, margin):
    ...
    world_cup = world_cup.sort_values(
        by=['Group', 'points'], ascending=False).reset_index()
...
rank["country_full"] = rank["country_full"].str.replace("IR Iran", "Iran").str.replace(
    "Korea Republic", "South Korea").str.replace("USA", "United States")
rank = rank.set_index(['rank_date']).groupby(['country_full'], group_keys=False).resample(
    'D').first().fillna(method='ffill').reset_index()
...
df = df.merge(rank, left_on=['date', 'home_team'],
              right_on=['rank_date', 'country_full'])
df = df.merge(rank, left_on=['date', 'away_team'], right_on=[
              'rank_date', 'country_full'], suffixes=('_home', '_away'))
...
X, y = df.loc[:, ['average_rank', 'rank_difference',
                  'point_difference', 'is_stake']], df['is_won']
...
model_lr, accuracy_lr, roc_auc_lr, coh_kap_lr = run_model(
...
model_dt, accuracy_dt, roc_auc_dt, coh_kap_dt = run_model(
    model_dt, X_train, y_train, X_test, y_test)

# MLPClassifier
print("MLPClassifier: ")
params_nn = {'hidden_layer_sizes': (
    30, 30, 30), 'activation': 'logistic', 'solver': 'lbfgs', 'max_iter': 500}
model_nn = MLPClassifier(**params_nn)
model_nn, accuracy_nn, roc_auc_nn, coh_kap_nn = run_model(
...
model_rf, accuracy_rf, roc_auc_rf, coh_kap_rf = run_model(
...
model_lgb, accuracy_lgb, roc_auc_lgb, coh_kap_lgb = run_model(
...
model_xgb, accuracy_xgb, roc_auc_xgb, coh_kap_xgb = run_model(
